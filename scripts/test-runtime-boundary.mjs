...
assert.match(rust, /window_target_at_cursor/);
assert.match(rust, /EnumWindows/);
assert.doesNotMatch(rust, /WindowFromPoint/);
assert.match(rust, /WDA_EXCLUDEFROMCAPTURE/);
assert.match(rust, /DwmFlush/);
assert.match(rust, /capture_virtual_region/);
...
assert.match(nativeRuntime, /windowTargetAtCursor/);
...
assert.match(nativeRuntime, /ShotHistoryEvent/);
...
assert.doesNotMatch(resultOverlay, /nativeRuntime\.listShotHistory\(\)/);
assert.doesNotMatch(resultOverlay, /findAvailableOverlayEntries/);
assert.match(resultOverlay, /kind === "updated"/);
assert.match(resultOverlay, /entriesRef/);
assert.match(resultOverlay, /transitioning/);
assert.match(resultOverlay, /PanelLeftOpen/);
assert.match(resultOverlay, /PanelRightOpen/);
...
assert.match(captureOverlay, /listDisplays\(\)/);
...
assert.match(captureOverlay, /CaptureOverlayOpenedPayload/);
assert.match(captureOverlay, /topologyReady/);
assert.match(captureOverlay, /setVirtualBounds/);
assert.match(captureOverlay, /payload\.displays/);
assert.match(captureOverlay, /hideCaptureOverlay/);
assert.match(captureOverlay, /showCaptureOverlay/);
assert.match(captureOverlay, /CaptureDimmer/);
assert.match(captureOverlay, /scale\.x/);
assert.match(captureOverlay, /event\.key === "Enter"/);
assert.match(captureOverlay, /Move over a window, click to capture it, or drag a region/);
...
assert.match(editorWindow, /beginPan/);
assert.match(editorWindow, /event\.button === 1/);
assert.match(editorWindow, /activeTool === "select"/);
assert.match(editorWindow, /min-w-full/);
...
assert.match(rust, /\(32\.0, 64\.0\)/);
assert.match(rust, /target_size/);
assert.match(rust, /window\.scale_factor\(\)/);
assert.match(rust, /ShotHistoryEvent/);
assert.match(rust, /kind: "created"/);
assert.match(rust, /kind: "updated"/);
assert.match(rust, /CaptureOverlayOpenedPayload/);
assert.match(rust, /displays: display_list/);
